...
engine:
  id: copilot
  version: "0.0.394"            # Pin specific CLI version
...
Use a pinned version when you need reproducible runs. This example uses Copilot CLI <code>0.0.394</code>, a known bundled version that makes the pinning example concrete. Check upstream gh-aw release notes or the engines reference when choosing another valid version to pin.
...
The <code>imports</code> field also supports an object form that separates workflow imports from APM package setup. The older <code>dependencies</code> key is deprecated.

```yaml
imports:
  aw:
    - "./shared/base-config.md"
    - "githubnext/agentics/shared/security.md@main"
  apm-packages:
